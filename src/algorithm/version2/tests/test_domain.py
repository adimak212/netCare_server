...
def main():
    ...
    print("Resources add:", (pc + switch).as_dict())
    print("Fits in rack:", (pc + switch).fits_in(rack))
    ...
    print("Template fits rack:", template.fits_bin_type("rack"))

    demand1 = DemandVector(pc=10, switch=3, router=1)
    demand2 = DemandVector(pc=2, switch=1, router=0)

    print("Demand add:", (demand1 + demand2).as_dict())
    print("Demand sub:", (demand1 - demand2).as_dict())
    print("Demand covers:", demand1.covers(demand2))
    ...
    print("Preferences normalized:", prefs.normalized_importance())
