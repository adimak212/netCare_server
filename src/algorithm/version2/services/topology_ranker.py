...
class TopologyRanker:
    def __init__(self, templates, bin_templates):
        ...
        self._solve_cache: Dict[Tuple, PatternSolveResult] = {}
    ...
    def rank(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for topology in self.topologies:
            try:
                ...
                topology_metrics = self._get_topology_metrics(topology)
                ...
                results.append({
                    ...
                    "waste": solve_result.waste_score,
                    "waste_breakdown": getattr(solve_result, "waste_breakdown", {}),
                    "rack_count": solve_result.rack_count,
                })

            except Exception as exc:
                results.append({
                    "key": getattr(topology, "key", "unknown"),
    ...
    def _solve_for_demand(
        ...
        cache_key = self._make_cache_key(topology_key, params, normalized, demand)
    ...
    def _get_all_patterns(self) -> list:
        ...
        for bin_type in self.bin_templates.keys():
            if bin_type not in self._patterns_cache:
                self._patterns_cache[bin_type] = self.pattern_generator.generate_for_bin_type(bin_type)

            all_patterns.extend(self._patterns_cache[bin_type])
    ...
    def _make_cache_key(
        ...
        return (
            ...
            tuple(sorted((str(k), str(v)) for k, v in normalized.items())),
            tuple(sorted((str(k), str(v)) for k, v in demand.as_dict().items())),
        )
    def _get_topology_metrics(self, topology) -> dict[str, float]:
        if hasattr(topology, "metrics"):
            return topology.metrics()

        if hasattr(topology, "get_metrics"):
            return topology.get_metrics()

        if hasattr(topology, "topology_metrics"):
            return topology.topology_metrics()

        raise AttributeError(f"{topology.__class__.__name__} has no metrics() / get_metrics() method")