...
class BeamState:
    def __init__(
        ...
        self.pattern_usage = pattern_usage
...
class BeamPatternSolver:
    """
    Beam Search solver for pattern selection.

    - Keeps top-K states at each iteration
    - Uses heuristic scoring during search
    - Uses same waste computation as greedy solver for final result
    """

    def __init__(self, patterns: List[Pattern], beam_width: int = 5, max_steps: int = 50):
    ...
    def solve(self, demand: DemandVector) -> PatternSolveResult:
        ...
        for _ in range(self.max_steps):
            ...
            for state in beam:
                ...
                # Use only top patterns (important for performance)
                for pattern in self.patterns[:10]:
                    multiplicity = self._compute_multiplicity(pattern.counts, state.remaining)
                    if multiplicity <= 0:
                        continue

                    new_state = self._apply_pattern(state, pattern, multiplicity)
            ...
            # Keep top-K states
            new_states.sort(key=self._score_state, reverse=True)
            beam = new_states[: self.beam_width]

            if all(state.is_done() for state in beam):
                break

        # Final selection (best heuristic state)
        best_state = max(beam, key=self._score_state)
    ...
    # Core Beam Logic
    # ========================
    def _apply_pattern(self, state: BeamState, pattern: Pattern, multiplicity: int) -> BeamState:
        new_state = deepcopy(state)

        used_counts = pattern.counts.scale(multiplicity)

        new_state.provided = new_state.provided + used_counts
        new_state.remaining = (new_state.remaining - used_counts).clamp_non_negative()

        new_state.used_resources += pattern.total_resources * multiplicity
        new_state.total_capacity += pattern.bin_capacity * multiplicity
    ...
    def _score_state(self, state: BeamState) -> float:
        # Coverage
        covered = state.provided.total_items()

        # Rack penalty
        rack_penalty = state.rack_count * 5.0

        # Waste approximation
        unused_u = max(state.total_capacity.u - state.used_resources.u, 0)
        unused_ports = max(state.total_capacity.ports - state.used_resources.ports, 0)

        waste_penalty = unused_u * 0.5 + unused_ports * 0.3

        # Future estimation
        remaining_items = state.remaining.total_items()
        future_penalty = remaining_items * 2.0
